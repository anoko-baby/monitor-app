...
4. アプリの「API資格情報」タブにある **API secret key(クライアントシークレット)** も控えておく(Webhookの署名検証に使うため。Admin APIアクセストークンとは別の値です)
5. `orders/create` Webhookを、URL `https://mxenfgoviwxnlhokfvwc.supabase.co/functions/v1/shopify-webhook` 宛に登録
6. 3つの値を **このチャットに貼らず**、ご自身のターミナルから設定してください:
   ```
   npx supabase secrets set SHOPIFY_STORE_DOMAIN="your-store.myshopify.com" SHOPIFY_ACCESS_TOKEN="shpat_..." SHOPIFY_WEBHOOK_SECRET="API secret key"
   ```
...
**実装メモ**
- コード一式は実装・デプロイ済みだが、Shopify認証情報(store domain・access token・webhook secret)が未設定のため実機での動作確認はまだ

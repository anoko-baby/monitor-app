...
## 利用規約同意画面を全項目必須のチェックボックスに変更、必須表示を赤い*マークに統一(2026-08-26)

- 実機フィードバック: 「利用同意の画面、本来同意してもらえないとモニター依頼できないので、全部Yesじゃないと同意して次へは押せないようにしてほしい。あとトグルだと同意かどうかわかりにくいから、同意する✔みたいなチェックボックスにした方がいいかも」「必須と文字で表記するのではなくて赤い*マークとかの方がいいかも」
- `app/consent.tsx`: EC掲載/SNS掲載/広告利用の3項目を、任意のON/OFFトグル(Switch)から、チェックボックス(`Ionicons`の`checkbox`/`square-outline`)に変更。3項目すべてチェックしないと「同意して次へ」ボタンが押せないようにした(仕様上、この同意が無いとモニター依頼自体ができないため)。各項目のラベルも「〜してもよい」から「〜することに同意する」という同意表明の文言に変更
...
## 提出フォームで大量アップロード・動画アップロード時に「待機中」のまま止まる不具合を修正(2026-08-26)

- 実機フィードバック: 「沢山アップロードしたときや、動画をアップロードしたときに、待機中のままになって、ログインし直したり何しても消えない、待機中のままでどうにもならない」
- 原因: アップロードに時間がかかる(大量選択・大きい動画)間にページ再読み込み/再ログインが発生すると、`app/submission-form.tsx`は下書き(AsyncStorage)からアップロード中だったファイルを復元する際、ステータスを一旦「待機中(pending)」に戻すだけで、その後誰も処理を再開していなかった。Wi-Fi接続待ちの場合(`wifiOnly`)はネットワーク状態が変わった時に自動再開する仕組みがあったが、それ以外の「待機中」はどのタイミングでも自動的にも手動(再試行ボタンは`エラー`状態のみ表示)にも再開されず、永久に止まって見える状態になっていた
- 対応: 下書きからファイルを復元した直後に、「待機中」のファイルに対して自動的にアップロード処理を再開するようにした(Dropboxのチャンクアップロードは元々セッションIDから途中再開できる仕組みのため、続きから再開される)
- `npx tsc --noEmit`通過。アプリコードのみの変更で、DB/Edge Functionの変更は無い

## 提出履歴画面から確認済みの提出を開けない不具合を修正(2026-08-26)

- 実機フィードバック: 「確認済に移動して、内容を見ることも再提出をすることもできないんだ」
- 案件詳細(`campaign-detail.tsx`)からは確認済みのタスクを開けるようになっていたが、原因はもう一つ別にあった: モニター側の「提出履歴」画面(`app/submission-history.tsx`。確認待ち/確認済みのタブがある一覧画面)の各行が、そもそもタップしても何も起きないただの`View`のままで、ステータスに関わらず(確認待ちであっても)提出フォームを開く導線が無かった
- 対応: 各行を`Pressable`にし、タップで該当タスクの提出フォーム(データ提出は`/submission-form`、SNS投稿は`/sns-submission-form`)を開くようにした
- `npx tsc --noEmit`通過。アプリコードのみの変更で、DB/Edge Functionの変更は無い

## 確認済みタスクへの追加提出でサムネイルが表示されない不具合を修正(2026-08-26)

- 実機フィードバック: 「提出した動画も画像もサムネイルが全然表示されない。動画もサムネイル出せる?」
- 動画サムネイル自体は元々実装済み(先頭1秒のフレームを画像化。ネイティブは`expo-video-thumbnails`、Web版は`<video>`+`<canvas>`で自前に取得)。表示されなかった原因は別にあった
- 原因: `20260826000004`で「確認済み(approved)後も追加提出できるように」モニター向けRLSを緩和した際、サムネイル用Storage(`thumbnails`バケット)のINSERTポリシーの更新を漏らしていた。確認済みタスクへの追加提出では、本体ファイルはアップロードできてもサムネイルのアップロードだけRLSで拒否されて失敗し、Web版はこの失敗を無視して提出自体は継続する作りのため、サムネイル無しのまま提出されていた
- あわせて、サムネイルアップロードは`upsert:true`で呼んでおり同名パスへの再アップロードは内部的にUPDATE扱いになるが、そもそもモニター向けのUPDATEポリシー自体が存在しなかった(同じファイル名で複数回提出した場合に同様の失敗が起こり得る)ため、こちらも追加した
...
## 写真・動画の上限を「1回の提出あたり」に変更、超過時に追加提出への案内を表示(2026-08-26)

- 実機フィードバック: 「問い合わせを減らすために、超える場合は追加提出で分けてご提出くださいみたいな表示をしてほしい」
- これまで上限判定は「既に提出済みの件数」も加算するタスク単位の累積カウントだったため、一度上限まで提出すると追加提出をしても事実上それ以上追加できない状態だった(せっかく追加提出を何度でもできるようにしたのに、上限判定が古いままだった)
- 対応: `app/submission-form.tsx`の上限判定を「今回この場で選択中の件数」のみで判定するように変更(提出済みの件数は数えない)。これにより、1回の提出では上限までだが、提出後にあらためて追加提出することで、実質何度でも分けて提出できるようになった
- 上限に達した際のエラーメッセージも、「◯点までです。超える分は、一旦ここまでで提出のうえ、追加提出で分けてご提出ください」という案内に変更した
- `npx tsc --noEmit`通過。アプリコードのみの変更で、DB/Edge Functionの変更は無い

...
**実装メモ(2026-07-10未明・実機確認待ち)**
- DBスキーマ・RLS・NativeWindテーマ・画面一式・`invite-register` Edge Functionまで実装・デプロイ済み
- `invite-register`のエラー系(不正なコード/短いパスワード/未入力)はサーバー側で動作確認済み。**正常系(実際の招待コードでの登録完結)は未確認** — 実機で管理者ログイン→招待発行→モニター登録の一連を試す必要あり
- 初期管理者アカウントは意図的に未作成(パスワードをAzusaさん自身に決めてほしいため)。Supabaseダッシュボード→Authentication→Add User でauthユーザーを作成し、`profiles`テーブルに`role='admin', status='active', auth_user_id=<作成したユーザーのUID>`の行を追加すれば管理者ログインできます
- git: `main`への直push は自動モードの安全装置でブロックされたため、`overnight-m2-wip`ブランチに退避してpush済み。起きたら中身を確認して `main` にmerge/pushしてください
...
1. Shopify管理画面 → 設定 → アプリと販売チャネル → 「アプリを開発」→ カスタムアプリを作成
2. スコープ設定で以下を許可(読み取りのみ、書き込み権限は付与しない):
   - `read_products`
   - `read_orders`
   - `read_customers`
   - `read_discounts`
3. アプリをインストールし、Admin API アクセストークンを発行(`shpat_...`という文字列)
4. `orders/create` Webhookを登録(設定URLはM4着手時にこちらで用意するEdge Function URLをお伝えします。先にトークンだけ発行しておいてもらえればOK)
5. アクセストークンは **このチャットに貼らず**、`npx supabase secrets set SHOPIFY_ACCESS_TOKEN="..." SHOPIFY_STORE_DOMAIN="your-store.myshopify.com"` の形でご自身のターミナルから設定してください(M1のDropboxの時と同じ進め方です)

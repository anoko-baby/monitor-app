...
**実装メモ**
- コード一式は実装・デプロイ済みだが、Shopify認証情報(store domain・API key・API secret)が未設定のため実機での動作確認はまだ
- Shopifyの仕様変更(2026年1月〜Dev Dashboard必須・client_credentialsでの都度トークン取得)に合わせて実装済み。`supabase/functions/_shared/shopify.ts` が共通のトークン取得処理

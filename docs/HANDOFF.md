...
Phase 1(MVP)のうち **M1〜M7が完了・実機確認済み**、**M8(通知+お知らせ配信)は実装済み・実機確認待ち**。次は **M9(TestFlight・限定公開配信+βテスト運用)**。
...
- M7: 検収・差し戻し+全提出一覧
- M8: 通知+お知らせ配信(実装メモは`docs/TODO.md`のM8セクション参照。**実機でのPush到達確認がまだ**。CRON_SECRETのvault登録もAzusaさんの対応待ち)

M9に入る前に、`docs/TODO.md`のM9セクションを見て、必要な準備(EAS Build設定、Apple Developer / Google Playアカウント)をAzusaさんに確認してから着手すること(これまでのマイルストーンも同じ進め方をしている)。
...
`docs/TODO.md`のM9セクション(TestFlight・限定公開配信+βテスト運用)を読み、EAS Build設定・Apple Developer / Google Playアカウントの準備状況をAzusaさんに確認してから着手する。
また、M8のPush通知は実機未確認のため、CRON_SECRETのvault登録(`docs/TODO.md`のM8実装メモ参照)をAzusaさんに依頼し、実機でのPush到達確認も別途行うこと。
